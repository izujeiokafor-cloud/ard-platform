...
export const performSmartSearch = async (query: string, ads: Ad[]): Promise<SearchResult> => {
  ...
  try {
    ...
    return { adIds: [], explanation: "Oga/Madam, an error occurred during search. Try again!" };
  }
};
...
export const performVisualSearch = async (imageB64: string, ads: Ad[]): Promise<SearchResult> => {
  ...
  const response = await ai.models.generateContent({
    ...
    contents: {
      parts: [
        imagePart,
        { text: `Identify the main item or service in this image. Then, from the following list of local ads, find the most relevant matches based on the item type, color, usage, and category.
  ...
  try {
    ...
    console.error("Failed to parse Gemini visual response", error);
    return { adIds: [], explanation: "We couldn't process your photo. Please try another one!" };
  }
};
